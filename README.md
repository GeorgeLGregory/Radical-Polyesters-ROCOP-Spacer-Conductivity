...
## **Contents**
Gaussian 16 `.out` files for optimised structures of P(GTEMPO-_alt_-anhydride) DP = 2 oligomer model. Anhydride spacers included:
...
Files are organized by oxidation state: 
- **Neutral diradical (TEMPO•/TEMPO•):** individual `.out` files for each anhydride, including NBO outputs where relevant.
- **Singly oxidised (TEMPO⁺/TEMPO•):** `singly-oxidised.zip` containing `.out` files for each anhydride.
- **Doubly oxidised (TEMPO⁺/TEMPO⁺):** `doubly-oxidised.zip` containing `.out` files for each anhydride.
